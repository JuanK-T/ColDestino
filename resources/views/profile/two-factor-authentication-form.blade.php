<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Agregue seguridad adicional a su cuenta mediante la autenticación de doble factor.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('Usted Ha habilitado la autenticación de dos factores.') }}
            @else
                {{ __('Usted No ha habilitado la autenticación de doble factor.') }}
            @endif
        </h3>
